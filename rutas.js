...
function initMap() {
    const map = new google.maps.Map(document.getElementById('map'), {
        zoom: 14,
        center: { lat: -16.409047, lng: -71.537451 }
    });
    ...
    window.mostrarRuta = function() {
        const rutaSelect = document.getElementById('rutaSelect');
        const rutaKey = rutaSelect.value;
        const ruta = Rutas[rutaKey];

        if (!ruta) {
            console.error(`No se encontró la ruta con clave: ${rutaKey}`);
            return;
        }

        const solicitud = ruta.solicitud;
        ...
        let detalleHTML = `<h2>${ruta.nombre} (${ruta.recorrido}) Paraderos</h2><ul>`;
        ruta.paradas.forEach(parada => {
            detalleHTML += `<li>${parada.nombre} (${parada.coordenadas.lat}, ${parada.coordenadas.lng})</li>`;
        });
        detalleHTML += '</ul>';
        rutaDetalle.innerHTML = detalleHTML;
    };
}
